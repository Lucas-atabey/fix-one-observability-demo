...
export class Services extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    // (Conserve un seul KubernetesManifest → impact négligeable sur le throttling)
    ...
    // X-Ray Agent SA (IAM Role for IRSA)
    ...
    // ===============================
    // REPLACEMENT 1: X-Ray via Helm
    // ===============================
    const xrayChart = new eks.HelmChart(this, 'XRayDaemon', {
      cluster,
      repository: 'https://aws.github.io/eks-charts',
      chart: 'aws-xray-daemon',
      release: 'aws-xray-daemon',
      namespace: 'default',           // pour conserver xray-service.default:2000
      ...
      values: {
        serviceAccount: {
          ...
          name: 'xray-daemon',
          annotations: {
            "eks.amazonaws.com/role-arn": xrayserviceaccount.roleArn
          }
        }
      }
    });
    ...
    // ===================================================
    // REPLACEMENT 2: AWS Load Balancer Controller via Helm
    // (plus de manifests SA/CRD/wait custom)
    // ===================================================
    const awsLoadBalancerControllerChart = new eks.HelmChart(this, "AWSLoadBalancerController", {
      cluster,
      ...
      values: {
        ...
        serviceAccount: {
          ...
          annotations: {
            "eks.amazonaws.com/role-arn": loadBalancerserviceaccount.roleArn
          }
        }
      }
    });
